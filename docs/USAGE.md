...
| **세션 복원** | 앱 시작 시 마지막 분석 결과 자동 복원 |
| **URL 필터** | 특정 URL 패턴만 분석 (예: `/api/user/*`) |
| **출력 스타일** | compact, normal, detailed 선택 |
| **분석 시작/취소** | 백그라운드에서 분석 실행, 진행률 표시. 분석 중 버튼이 취소로 변경 |
| **분석 타임아웃** | 기본 5분 후 자동 중단, 에러 다이얼로그 표시 |
...
| **설정 저장** | 최근 경로, URL 필터, 출력 스타일, 로그 크기 자동 저장 |
| **로그 폴더 열기** | ⚙ → 로그 폴더 열기 → 파일 탐색기에서 로그 폴더 열림 |
| **로그 크기 설정** | ⚙ → 로그 크기 설정 → 1MB / 5MB(기본) / 10MB 선택 |
| **로그 파일 삭제** | ⚙ → 로그 파일 삭제 → 모든 로그 파일 삭제 |
| **설정/세션 초기화** | ⚙ → 설정/세션 초기화 → 저장된 설정 및 분석 결과 삭제 |
