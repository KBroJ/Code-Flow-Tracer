...
> 최종 수정일: 2026-01-28 (에러 핸들링 GUI 테스트 → 버그 4건 수정)
...
### 2025-12-30 (월) ~ 2025-12-31 (화) - v1.1 기능 확장 🔄
...
- [x] TDD 단위 테스트 추가 (SessionManagerTest, SessionDataTest) ✅ 2025-12-31
- [x] WiX 설치 삭제 시 세션 폴더 정리 설정 추가 ✅ 2025-12-31
...
**GUI 테이블 영향도 탭 ([#30](https://github.com/KBroJ/Code-Flow-Tracer/issues/30))** ✅
> #23에서 분리된 GUI 전용 기능
- [x] 탭 전환 시 왼쪽 패널 변경 (CardLayout - 엔드포인트/테이블)
- [x] 테이블 목록 패널 추가 (검색 + 리스트)
- [x] TableImpactPanel 재구성 (브레드크럼 + CardLayout)
- [x] 테이블 선택 시 접근 정보 표시 (CRUD/URL/XML파일/SQL ID)
- [x] 더블클릭 드릴다운 (테이블 → 쿼리 상세)
- [x] 브레드크럼 네비게이션 (테이블명 클릭 → 뒤로가기)
- [x] "전체" 옵션 기본 선택 및 상세화면 표시
- [x] 세션 저장/복원 (탭, 엔드포인트, 테이블, 쿼리 상세 상태)
- [x] 사용자 테스트 및 버그 수정
- [x] **상세화면 실시간 검색** (RowFilter 기반 테이블 필터링)
- [x] **분석요약 탭별 동적 변경** (CardLayout - 클래스 통계 ↔ CRUD 통계)
- [x] **URL 필터 탭별 표시/숨김** (테이블 영향도 탭에서 URL 필터 숨김)
- [x] **마우스 뒤로가기 버튼 지원** (XBUTTON1 - 쿼리 상세에서 뒤로가기)
- [x] **SQL 필터 상태 유지** (필터 변경 시 현재 테이블/쿼리 선택 유지)
...
### 실무 테스트 이슈 수정 🔴 우선순위 높음

**에러 핸들링 강화 ([#34](ISSUES.md))** - ✅ 완료
- [x] SwingWorker 예외 처리 강화 (done()에서 get() 호출)
- [x] 에러 다이얼로그 표시 (사용자에게 오류 알림 + 로그 경로 안내)
- [x] 에러 로그 파일 생성 (~/.code-flow-tracer/logs/cft.log)
- [x] 분석 타임아웃 구현 (기본 5분, javax.swing.Timer)
- [x] 분석 취소 버튼 추가 (토글 방식: 분석 실행 ↔ 분석 취소)
- [x] CftLogger 클래스 구현 (java.util.logging 기반, 로테이션 5MB×3)
- [x] "로그 폴더 열기" 메뉴 추가 (설정 메뉴)
- [x] 로그 크기 설정 기능 (1MB/5MB/10MB 라디오버튼, 세션에 저장)
- [x] "로그 파일 삭제" 메뉴 추가 (FileHandler 잠금 해제 후 삭제)
- [x] 버그 수정: 설정 메뉴 팝업 캐시 → 매번 생성으로 변경
- [x] 버그 수정: SLF4J 한글 깨짐 → CftLogger로 통합
- [x] 버그 수정: 로그 삭제 시 파일 잠금 → closeHandlers() 추가
- [x] 버그 수정: 로그 삭제 직후 파일 재생성 → 삭제 후 logger.info() 호출 제거
- [x] 테스트 코드 작성 (CftLoggerTest 23개, SessionData/Manager +10개)
- [x] GUI 최종 테스트 완료

**HTTP Method 파싱 ([#31](ISSUES.md))**
- [ ] @RequestMapping method 속성 파싱
- [ ] RequestMethod.GET/POST/PUT/DELETE 변환
- [ ] 복수 메서드 {GET, POST} → "GET/POST" 처리

**내부 함수 호출 추적 ([#32](ISSUES.md))**
- [ ] 현재 클래스 컨텍스트 전달 (traceMethodCall)
- [ ] scope 빈 문자열/"this" 처리
- [ ] 옵션: --trace-internal-calls (기본 OFF)

**DAO/XML 매칭 개선 ([#33](ISSUES.md))**
- [ ] XML 기반 역매칭 (getAllSqlIds)
- [ ] 메서드 내 모든 문자열 리터럴 검사
- [ ] getSqlSession() 체이닝 지원
...
### 기술 블로그 (러너스하이 면접 대비)
- [x] 기술1: JavaParser로 Java 코드 분석하기 ([#55](https://kbroj9210.tistory.com/55))
...
- [x] 기술7: Swing으로 모던한 GUI 만들기 ([#61](https://kbroj9210.tistory.com/61))
- [x] 기술8: jpackage로 배포하기 ([#62](https://kbroj9210.tistory.com/62))
- [x] 기술9: Gson으로 세션 영속성 구현하기 ([#63](https://kbroj9210.tistory.com/63))
- [x] 기술10: CRUD 분석과 테이블 영향도 ([#64](https://kbroj9210.tistory.com/64))
- [ ] 러너스하이 마무리 회고

### 제출
- [x] 러너스하이 성장일지 작성 및 제출 ✅ 2026-01-18
...
| #26 | 테이블 영향도 세션 저장/복원 미동작 (windowClosing에서 saveSession 호출) | ✅ 해결 | 2026-01-12 |
| #27 | 호출흐름 스크롤 복원 타이밍 문제 (invokeLater 중첩으로 해결) | ✅ 해결 | 2026-01-12 |
| #28 | 테이블 "전체" 선택 시 상세화면 빈 화면 (호출 순서 변경) | ✅ 해결 | 2026-01-12 |
| #29 | SQL 필터 변경 시 테이블 선택 초기화 (상태 저장/복원으로 해결) | ✅ 해결 | 2026-01-12 |
| #31 | HTTP Method가 ALL로 표시 (@RequestMapping method 속성 미파싱) | 🔄 진행중 | - |
| #32 | 내부 함수 호출 미추적 (this.getCode() 등) | 🔄 진행중 | - |
| #33 | DAO/XML 매칭 불일치 (getSqlSession() 체이닝, 변수 참조 미지원) | 🔄 진행중 | - |
| #34 | 대용량 프로젝트 분석 시 앱 멈춤 + 에러 핸들링 부재 | ✅ 해결 | 2026-01-28 |
