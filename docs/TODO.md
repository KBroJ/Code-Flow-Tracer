...
> 최종 수정일: 2026-01-27 (실무 테스트 이슈 정리 및 에러 핸들링 요구사항 추가)
...
**에러 핸들링 강화 ([#34](ISSUES.md))** - 필수
- [ ] SwingWorker 예외 처리 강화 (done()에서 get() 호출)
- [ ] 에러 다이얼로그 표시 (사용자에게 오류 알림)
- [ ] 에러 로그 파일 생성 (~/.code-flow-tracer/logs/)
- [ ] 분석 타임아웃 구현 (기본 5분)
- [ ] 분석 취소 버튼 추가
...
| #34 | 대용량 프로젝트 분석 시 앱 멈춤 + 에러 핸들링 부재 | 🔴 신규 | - |
